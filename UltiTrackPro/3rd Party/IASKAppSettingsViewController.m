...
#pragma mark Ultitrack Reset
- (void)settingsViewController:(IASKAppSettingsViewController*)sender buttonTappedForSpecifier:(IASKSpecifier*)specifier {
	if ([specifier.key isEqualToString:@"ButtonDemoAction1"]) {
		UIAlertView *alert = [[[UIAlertView alloc] initWithTitle:@"Progress Reset" message:@"This will reset your day progress but keep your history" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"RESET",nil] autorelease];
		[alert show];
        ...
        //BandSetTableViewController *controller = [[BandSetTableViewController alloc] init];
        BandSetTableViewController *controller = [self.storyboard instantiateViewControllerWithIdentifier:@"bandSets"];
        
        controller.dataSource = [[DataHelper sharedManager] bandSets];
        [self.navigationController pushViewController:controller animated:YES];
	}
}
- (void)alertView:(UIAlertView *)alertView willDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (!buttonIndex == 0)  { [[DataHelper sharedManager] resetProgress]; }
}
@end
